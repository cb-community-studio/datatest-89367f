...
import itemFakerFactory from "./itemFakerFactory";
...
const ItemFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = itemFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("item").create(elem));
...
export default connect(null, mapDispatch)(ItemFakerDialogComponent);
