...
import orderFakerFactory from "./orderFakerFactory";
...
const OrderFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = orderFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("order").create(elem));
...
export default connect(null, mapDispatch)(OrderFakerDialogComponent);
