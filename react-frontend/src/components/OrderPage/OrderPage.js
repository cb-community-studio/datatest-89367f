...
const OrderPage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            .service("order")
            .find({ query: { $limit: 100 , $populate : ["ItemID","Name"] } })
            .then((res) => {
                let results = res.data;
                setInitData(results);
                 results = populateCollectionFields(results,"ItemID", "ItemID");
            results = populateCollectionFields(results,"Name", "name"); 
                setData(results);
            })
            .catch((error) => {
                console.log({ error });
    ...
    return (
        <div className="col-12 flex flex-column align-items-center">
            ...
            <OrderEditDialogComponent entity={initData[selectedEntityIndex]} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
