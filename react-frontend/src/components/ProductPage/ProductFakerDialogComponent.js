...
import productFakerFactory from "./productFakerFactory";
...
const ProductFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = productFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("product").create(elem));
...
export default connect(null, mapDispatch)(ProductFakerDialogComponent);
